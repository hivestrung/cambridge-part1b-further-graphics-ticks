...
float cubeUnion(vec3 p) {
  return min(cube(p - vec3(-3,0,-3)), sphere(p - vec3(-2,0,-2)));
}

float cubeDiff(vec3 p) {
  return max(cube(p - vec3(3,0,-3)), -sphere(p - vec3(4,0,-2)));
}

float cubeIntersect(vec3 p) {
  return max(cube(p - vec3(3,0,3)), sphere(p - vec3(4,0,4)));
}

float cubeBlend(vec3 p) {
  return smin(cube(p - vec3(-3,0,3)), sphere(p - vec3(-2,0,4)));
}

float SDF(vec3 p) {
  return min(min(cubeBlend(p), cubeUnion(p)), min(cubeDiff(p), cubeIntersect(p)));

}

vec3 getNormal(vec3 pt) {
  return normalize(GRADIENT(pt, SDF));
}
